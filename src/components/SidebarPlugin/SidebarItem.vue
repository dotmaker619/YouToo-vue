<template>
  ...
  >
    ...
    >
      ...
      <template v-else>
        <template v-if="link.comming">
          <el-tooltip content="Comming Soon" placement="top-start" >
            <i class="btn-tooltip" :class="link.icon"></i>
          </el-tooltip>
        </template>
        <template v-else>
          <i :class="link.icon"></i>
...
export default {
  ...
  computed: {
    ...
    isMenuActive() {
      if (this.$route && this.$route.path) {
        let matchingRoute = this.$route.path.startsWith(this.link.path)
        console.log('link, route', this.link.path, this.$route.path, matchingRoute)
...
.nav-wrapper > .nav > .nav-item > .nav-link.active {
  background: #f8f9fe !important;
  color: grey !important;
  border-radius: 0 !important;
  border-bottom: 4px solid #2889FC;
}
</style>
