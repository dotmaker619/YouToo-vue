<template v-slot="{ row }">
  <div>
    ...
    <div style="width:200px; height:40px;">
      <div style="width:50%; height:40px; float:left; padding: 0 14px; color: grey;">
        <router-link to="/events">Published</router-link>
      </div>
      <div style="width:50%; height:40px; float:right; padding: 0 30px; border-bottom: 4px solid #2889fc; ">
        Drafts
      </div>
    </div>
                    ...
                    <div class="d-flex align-items-center participant">
                      ...
                      <div class="d-flex align-items-center ml-3 eventHandle">
                        <a class="completion mr-3" v-if="row.status !== 'completed' && row.status !== 'cancelled'">
                          <router-link v-bind:to="'/eventdetail/' + row.id">
                        ...
                        <a class="completion mr-3">
                          <!-- <router-link @click="setPublished(10)" to="/events"> -->
                          <img style="width:19px; height:20px; cursor: pointer" @click="setPublished(row.id); $router.push('/events')" src="img/icons/common/drafts.png" />
                          <!-- </router-link> -->
                        </a>
                      </div>
                    </div>
...
export default {
    ...
    return {
      currentPage: 1,
        ...
        model_drafts: {
          ...
          time_drafts: '',
          kids_drafts: 'Kids',
          description_drafts: 'Description(age from to etc.)',
          deposit_drafts: '1.99$',
        },
      projectsPage: [],
  ...
  computed: {
    // ...mapState(['projects']),
    ...mapGetters({ projects: 'draftProject'}),
    totalPage() {
      return (
